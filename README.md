...
Конструктор класса принимает инстант брокера событий.\
Поля класса:
- `items: Map<string, number>` —  товары в корзине, где ключ — ID товара, значение — количество этого товара;
...
Методы класса:
- `updateOrderField(field: keyof IOrderDetails, value: string): void` обновляет поле данных заказа генерирует событие при изменении;
...
Методы:
- Сеттеры и геттеры для id, title, price.
...
Методы:
- Сеттеры для description, image и category, обновляющие содержимое соответствующих элементов.
...
- `'order:changed'`\
Генерируется при обновлении любого поля формы заказа (`OrderModel`).\
Действие: Обновить отображение формы — включить/выключить кнопку, показать/скрыть ошибки.
- `'order:errors'`\
Генерируется при невалидных данных в форме (`OrderModel`).\
Действие: Показать сообщения об ошибках в полях формы заказа.
- `'order:submit'`\
...
Действие: Закрыть модальное окно с содержимым формы, очистить форму, разблокировать прокрутку.
- `'form:input'`\
Генерируется при каждом вводе в поле формы (`'Form'`).\
Действие: Обновить соответствующее поле в `OrderModel`.
- '`success:close'`\
Генерируется при клике на кнопку "За новыми покупками!" окна успешного заказа (`Success`).\
Действие: Закрыть модальное окно, очистить данные заказа.